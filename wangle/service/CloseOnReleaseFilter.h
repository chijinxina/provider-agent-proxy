...
class CloseOnReleaseFilter : public ServiceFilter<Req, Resp> {
 public:
  explicit CloseOnReleaseFilter(std::shared_ptr<Service<Req, Resp>> service) : ServiceFilter<Req, Resp>(service) {}
  ...
  folly::Future<Resp> operator()(Req req) override
  {
    ...
    {
      return folly::makeFuture<Resp>(folly::make_exception_wrapper<std::runtime_error>("Service Closed"));
    }
  }
